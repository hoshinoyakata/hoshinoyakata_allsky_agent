...
async function loadStatus(force=false){
  try{
    ...
    if(b.ok){
      ...
      setText("sideTemp", `${b.temperature}℃`);
      setText("hum", `${b.humidity}%`);
      setText("press", `${b.pressure} hPa`);
      setText("bmeMsg", b.message || "BME280 正常");
    }else{
      setText("temp", "--");
      setText("sideTemp", "--");
      setText("hum", "--");
      setText("press", "--");
      setText("bmeMsg", b.message || "BME280未読込");
    }
    ...
    setText("windDir", `風向 ${w.direction || "--"} (${w.degrees ?? "--"}°)`);
    setText("rain", (s.rain || {}).label || "--");

    const sys = s.system || {};
    setText("uptime", sys.uptime || "--");
    setText("ip", sys.ip || "--");
    setText("cameraState", sys.camera || "--");
  }catch(e){
...
function refreshImage(){
  ...
  img.onerror = () => {
    viewer.classList.remove("hasImage");
    setText("empty", "画像読込失敗");
  };
...
async function recordVideo(){
  setText("message", "動画撮影中...");
  const res = await fetch("/api/video", {method:"POST"});
  const j = await res.json();
  setText("message", j.message || "動画処理完了");
}
...
function renderThumbs(items){
  ...
  el.innerHTML = list.map((name, i) => {
    const label = i === 0 ? "最新" : `${i*10}分前`;
    return `<div class="thumb"><img src="/images/${name}?thumb=${i}&ts=${Date.now()}"><small>${label}</small></div>`;
  }).join("");
}
